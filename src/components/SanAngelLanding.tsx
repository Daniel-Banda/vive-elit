...
export default function SanAngelLanding({ propertyId }: SanAngelLandingProps) {
    ...
    const heroImage =
        imagenes.length > 0
            ? imagenes[0]
            : "https://res.cloudinary.com/daxglaqjg/image/upload/v1768366125/IMG_6187_qf4jud.jpg";
    ...
    if (loading) {
        return (
            <div className="min-h-screen font-serif flex items-center justify-center text-slate-400 italic">
                Cargando Vive Elit...
            </div>
        );
    }
            ...
            <section className="relative w-full h-[40vh] min-h-[350px] flex items-end pb-12 mb-12">
                ...
                <div className="relative z-10 w-full max-w-5xl mx-auto px-4 sm:px-6 lg:px-8 flex flex-col md:flex-row md:justify-between md:items-end gap-4 text-white">
                    ...
                    <div className="text-left md:text-right mt-4 md:mt-0">
                        <p className="text-sm font-bold uppercase tracking-widest text-white/80 mb-1 drop-shadow">
                            Precio
                        </p>
                        <p className="text-4xl font-serif font-bold drop-shadow-lg">
                            ${priceFormatted} <span className="text-lg text-white/80 font-normal">MXN</span>
                        </p>
                    </div>
                </div>
            </section>
                            ...
                            <div className="flex gap-2 mt-4 overflow-x-auto pb-2 scrollbar-hide">
                                {imagenes.map((img: string, idx: number) => (
                                    <button
                                        ...
                                        className={`h-20 sm:h-24 aspect-[4/3] flex-shrink-0 rounded-xl overflow-hidden border-2 transition-all duration-300 ${idx === currentImageIndex
                    ...
                    <div className="grid grid-cols-2 md:grid-cols-4 gap-6 divide-y md:divide-y-0 md:divide-x divide-slate-100">
                        ...
                        {(property.Construccion || property.Superficie) && (
                            <div className="flex flex-col items-center text-center justify-center p-2 pt-6 md:pt-2">
                                ...
                                <p className="text-2xl font-serif text-slate-900 font-bold">
                                    {property.Construccion || property.Superficie} <span className="text-sm">m²</span>
                                </p>
                                <p className="text-xs text-slate-500 uppercase tracking-widest font-semibold mt-1">
                                    Medidas
                                </p>
                            ...
                            <div className="w-full md:w-3/4">
                                <ul className="space-y-3 text-slate-700 text-[15px]">
                                    <li>
                                        ...
                                        <span className="font-medium">{property.Tipo || "Casa"}</span>
                                    </li>
                                    {property.Recamaras && (
                                        <li>
                                            <span className="text-slate-500 mr-1">Recámaras:</span>{" "}
                                            <span className="font-medium">{property.Recamaras} recámaras</span>
                                        </li>
                                    )}
                                    {property.WC && (
                                        <li>
                                            <span className="text-slate-500 mr-1">Baños:</span>{" "}
                                            <span className="font-medium">{property.WC}</span>
                                        </li>
                                    )}
                                    {property.Parking && (
                                        <li>
                                            <span className="text-slate-500 mr-1">Estacionamiento:</span>{" "}
                                            <span className="font-medium">{property.Parking} autos</span>
                                        </li>
                                    )}
                                </ul>
                            </div>
                        ...
                        <div className="flex flex-col md:flex-row gap-6 md:gap-12">
                            ...
                            <div className="w-full md:w-3/4">
                                <div className="text-slate-700 text-[15px] leading-relaxed space-y-6">
                                    {(propertiesData as any)[propertyId]?.descripcion ? (
                                        <div className="whitespace-pre-line">
                                            {(propertiesData as any)[propertyId].descripcion}
                                        </div>
                                    ) : (
                                        <p>
                                            Disfruta de amplitud y estilo con esta increíble propiedad. Ideal tanto para
                                            residencia privada como para inversión, asegurando plusvalía y diseño de alta
                                            calidad en {property.Ubicacion || "una excelente ubicación"}.
                                        </p>
                                    )}
                                    ...
                                    <div>
                                        ...
                                        <ul className="space-y-1">
                                            {(propertiesData as any)[propertyId]?.caracteristicas ? (
                                                (propertiesData as any)[propertyId].caracteristicas.map(
                                                ...
                                                <>
                                                    {property.Recamaras && (
                                                        <li>- {property.Recamaras} recámaras amplias</li>
                                                    )}
                                                    {property.WC && <li>- {property.WC} baños equipados</li>}
                                                    {property.Parking && (
                                                        <li>- Cochera para {property.Parking} autos</li>
                                                    )}
